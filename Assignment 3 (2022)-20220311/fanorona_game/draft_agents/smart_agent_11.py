...
class AI(FanoronaPlayer):
    ...
    def __init__(self, color):
        ...
        self.total_nodes = 0
        self.game_depth = 4
        self.max_move_time = 300 #we'll make it 22 seconds
        self.max_time = 300
        self.succesive_moves_transposition_table = {}##!!!1

    def play(self, state, remain_time):  # look at playgame line 220 fanorona gui
        print("")
        # print(state.)
        print(f"Player SMART AGENT 11 {self.position} is playing.")
    ...
    def evaluate(self, state):
        ...
        score = (state.score[self.color.value] - state.score[-self.color.value])/(state.on_board[self.color.value]+state.on_board[-self.color.value])
...
def minimax_search(state, player, prune=True):
    ...
    _, action = max_value(state, -inf, inf, 0) #we set the subdepth at 1
...
# def iterative_minimax(state, player, prune=True):
#     for i in range(100):
#         action =