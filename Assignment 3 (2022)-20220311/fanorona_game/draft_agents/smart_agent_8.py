...
class AI(FanoronaPlayer):
    ...
    def play(self, state, remain_time):  # look at playgame line 220 fanorona gui
        print("")
        # print(state.)
        print(f"Player SMART AGENT 8 {self.position} is playing.")
    ...
    def max_value(self, state, alpha, beta,isMax):
        val = -inf
        action = None
        for a, s in self.sub_successors(state, isMax):
            if s.get_latest_player() == s.get_next_player():  # next turn is for the same player
                ...
                if value == None:
                    ...
                    v, a = self.max_value(s, alpha, beta,isMax)  # removed the  + 1 since the branchign is not that big
                    self.transposition_table_successors[board_hash] = (v, a)
                else:
                    v, a = value
            else:  # next turn is for the other one
                return self.amount_captured(state, isMax), a
            if v > val:
                val = v
                action = a
                if v >= beta:
    ...
    def min_value(self, state, alpha, beta):  # simulation of the other player
        ...
        for a, s in self.sub_successors(state, False):
            if s.get_latest_player() == s.get_next_player():  # next turn is for the same player
                ...
                return self.amount_captured(state, False), a
            if v < val:
                val = v
                action = a
                if v <= alpha:
    ...
    def best_branch_val(self, state, isMax):
        if isMax:
            ...
            value, _ = self.max_value(state, -inf, inf,True)
            return value
        else:  # we want to min since it's not the same player
            # value, _ = self.min_value(state, -inf, inf)
            # value = self.amount_captured(state, False)
            value, _ = self.max_value(state, -inf, inf, False)

            return value
        # value, _ = self.max_value(state, -inf, inf)

    def remove_useless_actions(self, actions, depth):
        length = len(actions)
        actions_kept = int(length * (1 - (depth-1) / (2 * self.depth_minmax)))
        if length <= actions_kept:
            return actions
        for cutoff_index in range(actions_kept, length):
            # we will keep the 10 most interesting moves
            # print(actions)
            # print(str(len(actions))  + str(cutoff_index))
            if actions[cutoff_index][2] == 1 and actions[cutoff_index][2] != 0:  # we need to check for good moves
                self.actions_cut += length - cutoff_index
                return actions[:cutoff_index -1]
            # the best will be the first we will remove max 1/3
        return actions

    def successors(self, state, isMax, depth):  # as a function of how many are taken,
        ...
        actions = FanoronaRules.get_player_actions(state, state.get_next_player())
        output = []
        oldstate_eval = self.amount_captured(state, isMax)
        ...
        for action in actions:
            ...
            if newstate:
                ...
                output.append((action, newstate, self.best_branch_val(newstate, isMax) - oldstate_eval)) #THE OTHER SIDE ALSO WANTS TO TAKE AS MUCH
                # output.append((action, newstate))
        ...
        #     # we check i f this is the same player so we want to max
        #     output.sort(key=lambda output: output[2], reverse=True)
        #     self.remove_useless_actions(output,depth)
        output.sort(key=lambda output: output[2], reverse=True)
        self.remove_useless_actions(output, depth)
        # else:# we want to min since it's not the same player
        #     output.sort(key= lambda output: output[2], reverse=False)
    ...
    def amount_captured(self, state, isUs):
        if isUs:
...
def minimax_search(state, player, prune=True):
    ...
    def max_value(state, alpha, beta, depth):
        ...
        val = -inf
        action = None
        list_of_successors = player.successors(state, True, depth)
    ...
    def min_value(state, alpha, beta, depth):  # simulation of the other player
        ...
        val = inf
        action = None
        list_of_successors = player.successors(state, False,depth)
    ...
    print("actions cut : " + str(player.actions_cut))
    return action
