...
from fanorona.fanorona_action import FanoronaActionType, FanoronaAction
MAX_SCORE = 22

class FanoronaRules(Rule):
    ...
    def is_legal_move(state, action, player): 
        ...
        Args:
            state (FanoronaState): A state object from the Fanorona game.
        ...
        if state.get_next_player() == player:
            if action['action_type'] == FanoronaActionType.MOVE:
                if state.get_board().get_cell_color(act_at) == Color(player):
                    effective_moves = FanoronaRules.get_effective_cell_moves(state, act_at)
    ...
    def get_effective_cell_moves(state, cell):
        ...
        Args:
            state (FanoronaState): The current game state.
        ...
        if board.is_cell_on_board(cell):
            possibles_moves = FanoronaRules.get_rules_possibles_moves(cell, board.board_shape)
            effective_moves = []
            for move in possibles_moves:
                if board.is_empty_cell(move):
                    effective_moves.append(move)
            if state.winmove is not None:  # EDIT: it is not mandatory to perform a combo
                effective_moves.append(cell)
    ...
    def make_move(state, action, player):
        ...
        approach = FanoronaRules.is_win_approach_move(at, to, state, player)
        remote = FanoronaRules.is_win_remote_move(at, to, state, player)
        ...
        if win:
            ...
            state.winmove = (at,to)
            state.combo_path.append(at)  # EDIT: verify combo circuit
        ...
        done = FanoronaRules.is_end_game(state) #return true or false
        return state, done
    ...
    def act(state, action, player): 
        ...
        """
        if FanoronaRules.is_legal_move(state, action, player):
            return FanoronaRules.make_move(state, action, player) #this will return a state
        else:
            return False
    ...
    def is_win_approach_move(at, to, state, player):
        ...
        if at == to:  # EDIT: it is not mandatory to perform a combo
            return captured
    ...
    def get_player_actions(state, player):
        ...
        Returns:
            List[FanoronaAction]: Contains all possible actions for a player at the given state.
        """
        ...
        if state.winmove is not None:
            if player == state.get_latest_player() == state.get_next_player():
                at, to = state.winmove
                moves = FanoronaRules.get_effective_cell_moves(state, to)
                if moves:
                    for move in moves:
                        if move == to:  # EDIT: it is not mandatory to perform a combo
                                actions.append(FanoronaAction(action_type=FanoronaActionType.MOVE, at=to, to=move))
                        elif move in empty_cells and move not in state.occuped and move not in state.combo_path:  # EDIT: verify circuit
                            if len(FanoronaRules.is_win_approach_move(to, move, state, player)) != 0:  # EDIT: generate 2 actions if both approach and remote are possible
                                actions.append(FanoronaAction(action_type=FanoronaActionType.MOVE, win_by="APPROACH", at=to, to=move))
                            if len(FanoronaRules.is_win_remote_move(to, move, state, player)) != 0:
                                actions.append(FanoronaAction(action_type=FanoronaActionType.MOVE, win_by="REMOTE", at=to, to=move))
                    return actions
        else:
            for piece in player_pieces:
                moves = FanoronaRules.get_effective_cell_moves(state, piece)
                if moves:
                    for move in moves:
                        if move in empty_cells:
                            ap = FanoronaRules.is_win_approach_move(piece, move, state, player)
                            rm = FanoronaRules.is_win_remote_move(piece, move, state, player)
                            if len(ap) + len(rm) == 0:
                                simpleacts.append(FanoronaAction(action_type=FanoronaActionType.MOVE, at=piece, to=move))
                            else:  # EDIT: generate 2 actions if both approach and remote are possible
                                if len(ap) > 0:
                                    winacts.append(FanoronaAction(action_type=FanoronaActionType.MOVE, win_by="APPROACH", at=piece, to=move))
                                if len(rm) > 0:
                                    winacts.append(FanoronaAction(action_type=FanoronaActionType.MOVE, win_by="REMOTE", at=piece, to=move))
            if len(winacts) != 0:
                return winacts
            else:
                return simpleacts
    ...
    def moment_player(state, players):
        ...
        if player == state.get_latest_player():
            if state.winmove is not None:
                action = FanoronaRules.get_player_actions(state, player)
                if players[player].allow_combo is True and len(action) != 0:
                    if len(action) == 1 and action[0].get_action_as_dict()['action']['at'] == action[0].get_action_as_dict()['action']['to']:  # EDIT: it is not mandatory to perform a combo
                        state.winmove = None 
                        state.combo_path = []  # EDIT: verify circuit
                        state.set_next_player(player * -1)
                        return player * -1
                    else:
                        return player
    ...
    def random_play(state, player):
        ...
        """
        import random
        actions = FanoronaRules.get_player_actions(state, player)
        if len(actions) == 0:
            choice = None
        else:
    ...
    def is_player_stuck(state, player):
        ...
        if state.winmove is None: 
            return len(FanoronaRules.get_player_actions(state, player)) == 0
    ...
    def is_end_game(state): 
        ...
        """
        if FanoronaRules.is_player_stuck(state, state.get_next_player()) or FanoronaRules.is_boring(state) : #
            return True
        if state.get_latest_player() is not None: 
